...
//
    ...
    open class DelegateProxy<P: AnyObject, D: AnyObject>: _RXDelegateProxy {
        ...
        public init<Proxy: DelegateProxyType>(parentObject: ParentObject, delegateProxy: Proxy.Type)
            ...
            _currentDelegateFor = delegateProxy.currentDelegate(for:)
            _setCurrentDelegateTo = delegateProxy.setCurrentDelegate(_:to:)
        ...
        private func checkSelectorIsObservable(_ selector: Selector) {
            ...
            guard ((forwardToDelegate() as? NSObject)?.responds(to: selector) ?? false) || voidDelegateMethodsContain(selector) else {
