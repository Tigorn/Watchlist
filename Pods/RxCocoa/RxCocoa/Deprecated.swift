...
extension ObservableConvertibleType {
    ...
    public func asSharedSequence<S>(sharingStrategy _: S.Type = S.self, onErrorJustReturn: E) -> SharedSequence<S, E> {
        let source = asObservable()
     ...
     */
    @available(*, deprecated, message: "Please use conversion methods to some SharedSequence specialization.")
    public func asSharedSequence<S>(sharingStrategy _: S.Type = S.self, onErrorDriveWith: SharedSequence<S, E>) -> SharedSequence<S, E> {
        let source = asObservable()
            .observeOn(S.scheduler)
            .catchError { _ in
     ...
     */
    @available(*, deprecated, message: "Please use conversion methods to some SharedSequence specialization.")
    public func asSharedSequence<S>(sharingStrategy _: S.Type = S.self, onErrorRecover: @escaping (_ error: Swift.Error) -> SharedSequence<S, E>) -> SharedSequence<S, E> {
        let source = asObservable()
            .observeOn(S.scheduler)
            .catchError { error in
